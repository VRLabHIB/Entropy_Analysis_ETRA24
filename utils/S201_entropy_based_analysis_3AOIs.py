...
def calculate_entropy_measures_3AOIs(project_path, AOI_lst, datasets):
    data_path = project_path + '\\data\\'

    AOI_lst_small = ['Teacher', 'PresentationBoard', 'Student']
    ...
    for i in range(0, len(trans_lst)):
        ...
        dfT['Source'] = np.where(dfT['Source'].isin(stud_lst), 'Student', dfT['Source'])
        dfT['Target'] = np.where(dfT['Target'].isin(stud_lst), 'Student', dfT['Target'])

        dfN['Target'] = np.where(dfN['Target'].isin(stud_lst), 'Student', dfN['Target'])
        ...
        for start in interval_starts:
            ...
            if np.logical_or(len(dfTs) != 0, len(dfNs) != 0):
                ...
                if len(dfTs) != 0:
                    ...
                    dfTs = fill_missing_nodes(dfTs, AOI_lst_small, 'edges')

                    G = nx.from_pandas_edgelist(dfTs, source='Source', target='Target', edge_attr=['Weight'],
                                                create_using=nx.DiGraph())
                    P = nx.to_numpy_array(G, nodelist=AOI_lst_small, weight='Weight')
                ...
                dfNs = fill_missing_nodes(dfNs, AOI_lst_small, 'nodes')
                dfNs['Target'] = dfNs['Target'].astype("category")
                dfNs['Target'] = dfNs['Target'].cat.set_categories(AOI_lst_small)
        ...
        df.to_csv(data_path + '{}_{}_entropy_3AOIs_30s.csv'.format(today, datasets), index=False)
    print(' ')
